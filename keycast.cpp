...
BOOL disableShow = FALSE;     // 是否禁用按键/鼠标操作显示
...
void saveSettings() {
    ...
    writeSettingInt(L"disableShow", disableShow);
    updateAutoRun();
...
void loadSettings() {
    ...
    disableShow = GetPrivateProfileInt(L"KeyCastOW", L"disableShow", 0, iniFile);
...
void renderSettingsData(HWND hwndDlg) {
    ...
    CheckDlgButton(hwndDlg, IDC_DISABLE_SHOW, disableShow ? BST_CHECKED : BST_UNCHECKED);
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
                case IDC_STATIC_ComBIne:
                    ...
                    return TRUE;
                case IDC_STATIC_Repeat:
                    MessageBox(hwndDlg,
                        L"关闭此功能将影响整个系统的按键捕获。\n\n"
                        L"关闭此项可用于修复 K380 等蓝牙键盘连接不稳定导致的按键重复问题，"
                        L"但可能会影响方向键的持续性。",
                        L"按住视为重复说明",
                        MB_OK | MB_ICONINFORMATION);
                    return TRUE;
                ...
                case IDOK:
                    ...
                    disableShow = (BST_CHECKED == IsDlgButtonChecked(hwndDlg, IDC_DISABLE_SHOW));
                    tcModifiers = 0;
        ...
        LPSTR lpszArgs, int nWinMode)
{
    ...
    WCHAR *fileName = wcsrchr(iniFile, L'\\');
    if(fileName != NULL) {
        *(fileName + 1) = L'\0';
        wcscat_s(iniFile, MAX_PATH, L"KeyCastOW-zh-CN.inf");
    } else {
        wcscpy_s(iniFile, MAX_PATH, L"KeyCastOW-zh-CN.inf");
    }
#ifdef _DEBUG
