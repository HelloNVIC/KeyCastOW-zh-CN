...
- 支持自定义按键标签的字体、颜色、透明度、边框、圆角、显示行数和位置
- 支持独立配置屏幕角落品牌标识的文本、字体、背景颜色、文本颜色、透明度、字号和背景大小
- 支持快捷键开启/关闭输入捕获
...
3. **品牌标识独立样式配置**
   ...
   - 屏幕角落的品牌标识不再复用按键标签样式，可单独设置文本、字体、背景颜色、文本颜色、背景透明度、字号和背景大小。
   - 品牌标识样式会随其他配置一起写入 INI 文件，重启后自动恢复。
   - 移除了品牌标识文字的高光/阴影效果，显示更简洁。
